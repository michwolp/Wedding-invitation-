export const MESSAGES = {
  he: {
    need: 'רק צריך שם וטלפון ואפשר לשלוח',
    sending: 'שולחים…',
    okYes: 'התקבל! מחכים לראות אתכם 🤍',
    okNo: 'התקבל, תודה שעדכנתם 🤍',
    err: 'משהו השתבש בשליחה — נסו שוב עוד רגע',
  },
  en: {
    need: 'Just a name and phone number and you can send',
    sending: 'Sending…',
    okYes: "Got it! Can't wait to see you 🤍",
    okNo: 'Got it, thanks for letting us know 🤍',
    err: 'Something went wrong — please try again in a moment',
  },
  ru: {
    need: 'Нужны только имя и телефон',
    sending: 'Отправляем…',
    okYes: 'Принято! Ждём вас 🤍',
    okNo: 'Принято, спасибо, что сообщили 🤍',
...
export const LABELS = {
  en: {
    ...
    '#details .btnrow a:nth-child(1)': 'Waze',
    '#rsvp .sec-title': 'RSVP',
    ...
    '.pickup-q': 'Would you like a shuttle?',
    '[data-pk=want-no]': "No, we'll get there on our own",
    '[data-pk=want-yes]': 'Yes',
    '[data-pk=from-q]': 'From where?',
    '[data-pk=city-tlv]': 'Tel Aviv',
    '[data-pk=city-rhv]': 'Rehovot',
    '[data-pk=dir-q]': 'Which way?',
    '[data-pk=dir-to]': 'There only',
    '[data-pk=dir-from]': 'Back only',
    '[data-pk=dir-both]': 'There & back',
    '[data-pk=time-q]': 'What time back?',
    '[data-pk=ret-before]': 'Before the after-party',
    '[data-pk=ret-after]': 'At the end of the night',
    '[data-pk=ret-note]': '* Before the after-party is around <span class="t">00:00</span>',
    ...
    '#gifts .btnrow a:nth-child(1)': 'Bit',
  ...
  ru: {
    ...
    '.pickup-q': 'Нужен трансфер?',
    '[data-pk=want-no]': 'Нет, доберёмся сами',
    '[data-pk=want-yes]': 'Да',
    '[data-pk=from-q]': 'Откуда?',
    '[data-pk=city-tlv]': 'Тель-Авив',
    '[data-pk=city-rhv]': 'Реховот',
    '[data-pk=dir-q]': 'В какую сторону?',
    '[data-pk=dir-to]': 'Только туда',
    '[data-pk=dir-from]': 'Только обратно',
    '[data-pk=dir-both]': 'Туда и обратно',
    '[data-pk=time-q]': 'Во сколько обратно?',
    '[data-pk=ret-before]': 'До афтепати',
    '[data-pk=ret-after]': 'В конце вечера',
    '[data-pk=ret-note]': '* До афтепати — около <span class="t">00:00</span>',
    ...
    '.rsvp-edit': 'Изменить ✏️',
    _name1: 'Михаль ',
    _name2: ' Двир',
    _title: 'Михаль ♥ Двир — 16.10.26',
    _dir: 'ltr',
  },
};
...
export const HE_SINGULAR = {
  m: {
    ...
    '.pickup-q': 'רוצה הסעה?',
    '[data-pk=want-no]': 'לא, אגיע עצמאית',
    '.seeyou': 'נשמח לראותך!',
  },
  f: {
    '.opening': 'מתרגשים להזמין אותך לחתונה שלנו!',
  ...
  plural_f: {
    ...
    '.pickup-q': 'רוצות הסעה?',
    '[data-pk=want-no]': 'לא, נגיע עצמאית',
    '.seeyou': 'נשמח לראותכן!',
  },
};
...
export function getMessage(lang, key, guestForm) {
  if (lang === 'he' && guestForm && MSG_HE_SINGULAR[guestForm]?.[key] != null) {
    return MSG_HE_SINGULAR[guestForm][key];
  }
  return MESSAGES[lang]?.[key] ?? MESSAGES.he[key];
}

export function getErrorMessage(lang, code) {
  const table = ERROR_MESSAGES[lang] || ERROR_MESSAGES.he;
